...
## Continues variable - interest rate 
...
d$int_groups = cut(d$int_rate, 5)

woedf <- woe('int_groups', d)
...
$$ \mathbb E[Y|X] = logit^{-1}(z) = logistic(z) = \dfrac{1}{1 + e^{-z}}$$

## Estimating the coefficients

The way we obtain the coefficients $\beta$ is by using a procedure called maximum likelihood (ML). 

$$\Pi_{i=1}^{n} \left[ P(Y=1|X) ^ {y_{i}} (P(Y=0|X)^{1 - y_{i}})  \right] \rightarrow max$$

$$L(\beta | Y, X) = \Pi_{i=1}^{n} \left[ logistic(z) ^ {y_{i}} ((1 - logistic(z))^{1 - y_{i}})  \right]$$
